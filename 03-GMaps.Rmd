...
titovResults <- ImportData(filename = "titovResults") %>% filter(type=="route", country != "Italy") %>% distinct(lon, lat) 
marsalaTitaResults <- ImportData(filename = "marsalaTitaResults" )%>% filter(type=="route", country != "Italy") %>% distinct(lon, lat) 
TitoGmapsResults <- bind_rows(titovResults, marsalaTitaResults)

titovResultsFiltered <- titovResults 
  

```

Out of curiosity, how many new locations have been found through the second batch of queries (Marsal Tito) that had not been found earlier (querying fot 'Titov)?

```{r}
dplyr::setdiff(marsalaTitaResults, titovResults) %>% nrow()
```
...
mapTonerLite + 
  stat_density2d(
    aes(x = lon, y = lat, fill = ..level..,  alpha = ..level..),
    size = 2, bins = 6, data = titovResultsFiltered,
    geom = "polygon"
)
```
